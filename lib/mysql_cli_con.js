...
MySqlCliCon.prototype.deleteFromDB = function ( job ) {
 		...
 		mcon.query("USE `"+config.get('Database.dbname')+"`" + ';DELETE FROM `printer_jobs` WHERE id = ' + job.id, function(err, rows, fields) {
...
MySqlCliCon.prototype.mysqlSearchForPrintJobs = function () {
		...
		var pMapPrefix = this.printerMapPrefix;
 		mcon.query("USE `"+config.get('Database.dbname')+"`" + ';SELECT * FROM printer_jobs WHERE site_alias IN (' + self.__getSiteAliasListForSql() + ')', function(err, rows, fields) {
		  if (err) throw err;
		  if ( rows.length ) {
...
MySqlCliCon.prototype.loadSitePrinters = function () {
	...
	mcon.query("USE `"+config.get('Database.dbname')+"`" + '; SELECT * FROM sites WHERE machine_uuid = ?', [newClient.uuid], function(err, rows, fields) {
		if (err) throw err;		  
		_.each(rows, function( siteObj ) {
			newSite = site.createSite( siteObj );
	...
	function loadPrintersPerSite ( site ) {
		console.info("cargando impresoras de " +site.alias);

		mcon.query('USE `' + site.getDbTenant() + '`; SELECT * FROM printers',  function(err, printrows, fields) {
			if (err) throw err;		  
